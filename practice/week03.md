...
# 💡 핵심 원리 설명
1. 나머지 연산자($%$)의 마법
리스트의 길이가 7일 때, color_index가 6(마지막 색상)에서 1이 더해지면 7이 됩니다. 이때 7 % 7을 계산하면 결과는 0이 됩니다. 덕분에 리스트 범위를 벗어나 에러가 나지 않고 자연스럽게
첫 번째 색상으로 되돌아갑니다. 게임 개발에서 인벤토리 슬롯이나 슬라이드 메뉴를 만들 때 정말 자주 쓰이는 기법이에요!
